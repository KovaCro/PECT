...
# feasibility and soft cost if we were to move event to timeslot/room
# if some other event is in timeslot/room, they're being swapped
type Pectn = tuple[int, int, int, int, int]
        ...
        for event in unscheduled_events:
            ...
            for ts in valid_timeslots:
                for room in valid_rooms:
                    ...
                    if np.any((solution[successors, 0] != -1) & (solution[successors, 0] < ts)):
                        continue
    ...
    if moves[2]:
        for i, e1 in enumerate(scheduled_events):
            for j in range(i + 1, len(scheduled_events)):
                ...
                if np.any((solution[successors_e1, 0] != -1) & (solution[successors_e1, 0] < ts2)):
                    continue
                successors_e2 = np.where(before[e2])[0]
                if np.any((solution[successors_e2, 0] != -1) & (solution[successors_e2, 0] < ts1)):
                    continue
